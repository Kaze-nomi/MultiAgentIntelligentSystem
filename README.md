...
- **ProjectContextWriter** - компонент для записи и управления контекстом проекта
...
Архитектура использует паттерны проектирования: Builder для поэтапного создания объектов, Facade для упрощения интерфейсов взаимодействия с подсистемами, и Strategy для выбора алгоритмов поведения агентов в зависимости от задач.
...
rectangle "ProjectContextWriter" as contextwriter {
  (Запись контекста)
  (Управление данными)
  (Интеграция)
}
...
contextwriter --> mcp : Контекст/Данные
...
mcp --> contextwriter : Контекст проекта
...
contextwriter --> github : Контекст/История
...
n8n --> docs : Авто-док.
n8n --> contextwriter : Интеграция данных
...
contextwriter --> prometheus : Метрики
...
grafana --> contextwriter : Дашборды
...
| ProjectContextWriter | 8006 | API управления контекстом проекта |
...
1. **Проверка здоровья агентов:**
   ...
   curl http://localhost:8006/health  # ProjectContextWriter
   ```
...
#### ProjectContextWriter (Запись контекста проекта)
- **Порт:** 8006
- **API:** http://localhost:8006
- **Назначение:** Управление и запись контекста проекта, интеграция данных
...
4. Система автоматически:
   ...
   - Запишет контекст проекта через ProjectContextWriter
...
4. Система автоматически:
   ...
   - Обновит контекст проекта через ProjectContextWriter
...
│   └── project_context_writer/ # Компонент записи контекста
