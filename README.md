...
- **OpenRouter MCP Server** - прокси-сервер для взаимодействия с языковыми моделями
...
|-----------|------|------------|
| OpenRouter MCP Server | 8000 | API для взаимодействия с LLM |
...
**Симптомы:** Ошибки при выполнении `docker-compose up -d`

**Решения:**
1. Проверьте версию Docker и Docker Compose:
   ```bash
   docker --version
...
2. Убедитесь, что все порты свободны:
   ```bash
   netstat -tulpn | grep -E ':(8000|8001|8002|8003|8004|8005|5678|9090|3000)'
   ```
3. Проверьте наличие файла `.env` с корректными параметрами

### Проблема: Агенты не отвечают на запросы

**Симптомы:** Ошибки при обращении к API агентов

**Решения:**
1. Проверьте здоровье контейнеров:
   ```bash
   docker-compose ps
   ```
2. Проверьте логи конкретного агента:
   ```bash
   docker-compose logs -f [имя_агента]
   ```
3. Убедитесь, что `OPENROUTER_API_KEY` корректен и активен

### Проблема: Нет доступа к веб-интерфейсам

**Симптомы:** Браузер не может открыть интерфейсы n8n, Grafana или Prometheus
...
2. Проверьте доступность портов:
   ...
   ```
3. Проверьте логи соответствующих сервисов

### Проблема: Недостаточно ресурсов

**Симптомы:** Система работает медленно или контейнеры перезапускаются
...
1. Проверьте использование ресурсов:
   ...
   docker stats
   ```
2. Увеличьте ресурсы Docker (если возможно)
3. Остановите неиспользуемые контейнеры

### Проблема: Проблемы с CloudPub туннелем

**Симптомы:** Внешние вебхуки не работают

**Решения:**
1. Проверьте токен CloudPub в файле `.env`
2. Проверьте логи CloudPub:
   ```bash
   docker-compose logs -f cloudpub
   ```
3. Убедитесь, что сервис n8n работает корректно

## 🛑 Остановка системы

Для полной остановки системы выполните:

```bash
docker-compose down
```

Эта команда остановит и удалит все контейнеры, но сохранит данные в томах.
...
**Внимание:** Эта команда удалит все данные, включая конфигурацию n8n, историю метрик Prometheus и дашборды Grafana.

Для перезапуска системы после остановки:
...
├── openrouter_mcp/            # MCP сервер для OpenRouter
