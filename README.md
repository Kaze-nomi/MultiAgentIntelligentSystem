...
Недавно Documentation Agent был расширен функциональностью для получения погодных данных через интеграцию с OpenWeatherMap API, используя паттерны Adapter, Strategy и Repository. Новые компоненты включают WeatherService для бизнес-логики и WeatherData для модели данных на базе Pydantic.
...
- **Documentation Agent** - агент для генерации и управления документацией, включая API для получения погоды
...
docs --> mcp : Док./Обновления/Погода
...
| Documentation Agent | 8005 | API документации и погоды |
...
# API ключ для OpenWeatherMap (для погодного API)
OPENWEATHERMAP_API_KEY=your_openweathermap_api_key_here
```

**Важно:** Параметры `OPENROUTER_API_KEY`, `DEFAULT_MODEL` и `OPENWEATHERMAP_API_KEY` являются обязательными для работы системы.
...
- **Назначение:** Генерация и управление документацией, включая API для получения погодных данных
...
### Пример 4: Получение погодных данных

Documentation Agent теперь поддерживает API для получения погоды. Пример запроса:

```bash
curl -X GET "http://localhost:8005/weather?city=Moscow" \
...
│   │   ├── interfaces/         # Интерфейсы для погодного API
│   │   ├── data/               # Модели данных
...
Для настройки погодного API в Documentation Agent добавьте `OPENWEATHERMAP_API_KEY` в файл `.env`. Компоненты WeatherService и WeatherData используют паттерны Adapter для интеграции с внешним API, Strategy для выбора провайдера и Repository для доступа к данным.

## 📡 API

### Documentation Agent API

- **GET /weather** - Получить погодные данные для указанного города.
  - Параметры: `city` (строка, обязательный)
  - Ответ: JSON с полями `city`, `temperature`, `description`, `humidity`
...
response = requests.get("http://localhost:8005/weather", params={"city": "Moscow"})
data = response.json()
